...
fn invoic_minimal_fixture_validates_against_schema() {
    let errors = collect_validation_error_flags("valid_invoic_d96a_minimal.edi");
    assert!(
...
fn invoic_full_fixture_validates_against_schema() {
    let errors = collect_validation_error_flags("valid_invoic_d96a_full.edi");
    assert!(
        errors.iter().all(|flag| !*flag),
        "unexpected validation errors"
    );
...
fn invoic_missing_bgm_fixture_reports_validation_errors() {
    let errors = collect_validation_error_flags("invalid_invoic_d96a_missing_bgm.edi");
    assert!(
