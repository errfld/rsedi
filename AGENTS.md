...
- 2026-02-17 (`#82` ORDRSP track): Adding message-type integration tests under `edi-validation` requires `edi-adapter-edifact` as a crate dev-dependency; otherwise new fixture-based parser+schema tests fail to compile.
- 2026-02-17 (`#82` SLSRPT track): D96A fixture schemas that model composite EDIFACT elements must keep those elements non-mandatory in this validator, because mandatory checks currently inspect scalar element values and not composite child components.
- 2026-06-08 (`#122`): Mapping diagnostics are most useful when DSL-only linting stays runtime-shape focused, with optional schema-aware checks behind an explicit `--schema` flag. This catches selector/runtime compatibility issues without requiring schemas for every lint invocation, while still suggesting likely EDIFACT segment typos when schema metadata is available.
- 2026-06-10 (`#125`): Mapping aggregate rules should operate over `find_collection` node selections and preserve scalar output types where possible (`count` -> integer, numeric extrema/sum -> decimal, first/last -> original value). Runtime diagnostics/errors need to include the aggregate op, source path, and offending input value because aggregate failures otherwise lack rule context.
